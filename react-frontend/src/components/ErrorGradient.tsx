...
const PctAvailGradient: React.FC<Props> = ({featureInfos, group, showTitle=true, height}:Props) => {
    // extracting the data
    const data = featureInfos.map(f=>f.imputation_error).sort().filter(v=>v!==null)
    ...
    const max_error = 1;
    const color_inbetween = d3.scaleQuantize(color_redYGreen.reverse()).domain([0,max_error]);
 
    const color= (val:number|null)=>{ 
        if (val === null) return "#0000FF"
        // if (val<0){
        //     return "#00FF00"
        // }
        // else if (val>1){
    ...
    //                }</svg>
    //            </div>)               <svg className="cluster"><circle cx={10} cy={8} r={7} fill={cluster_color(featureInfo.group_id)} ></circle></svg> {showTitle && <span>{title}</span>
